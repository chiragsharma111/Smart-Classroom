...
app.get('/login',function(req,res){
	res.send(jade.renderFile('login.jade',{"login_url" : "http://"+servip+":3000/login"}));
...
app.get('/testing/api',function(req,res){
	if(req.session && req.session.auth == true){
		res.send(jade.renderFile('api.jade',{"login_url" : "http://"+servip+":3000/testing/api"}));
		//res.sendFile(__dirname + '/api.html');
	}
	else{
		res.redirect("http://"+servip+":3000/login");
	}
})
	...
	if(req.session && req.session.auth == true){
		con.query("SELECT * FROM "+req.body.api+" ;", function (errr, results,fields) {
			...
			var html = jade.renderFile('Data.jade',{
    			items: results,"login_url" : "http://"+servip+":3000"
			});
			res.send(html);
		});
	}
...
app.get('/testing/gsearch',function(req,res){
	if(req.session && req.session.auth == true){
		res.send(jade.renderFile('gsearch.jade',{"login_url" : "http://"+servip+":3000/testing/gsearch"}));
...
app.post('/testing/gsearch',function(req,res){
	if(req.session && req.session.auth == true){
		...
		var result={"Rank":i,"Title":title,"Link":"http://"+servip+":3000/redirect/"+url};
		disp.push(result);
		console.log(i);
		console.log(title);
		console.log(url);
...
app.get("/ssh",function(req,res){
	if(req.session && req.session.auth == true){
		res.send(jade.renderFile('VM.jade',{"login_url" : "http://"+servip+":3000/ssh"}));
...
app.post("/ssh",function(req,res){
	...
	console.log("Session Confirmed");
		...
		var run = execSync("vboxmanage startvm Test --type headless");
		var ping = execSync("ping 192.168.43.214 -c 10");
		console.log("POST Request Processing");
			...
			res.redirect("http://"+servip+":3000/ssh");
		}
	}
	else{
		res.redirect("http://"+servip+":3000/login");
...
io.on('connection', function(socket) {
  ...
  conn.on('ready', function() {
    ...
    exec("vboxmanage controlvm Test savestate");
